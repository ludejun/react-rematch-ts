...
export default function(ua) {
  ...
  if (uaData.device.type === 'mobile' || uaData.device.type === 'tablet') {
    ...
    // 针对三星已经匹配出的数据做处理
      ...
      uaData.device.manufacturer.toLowerCase() === 'samsung' &&
      uaData.device.model
    ) {
    ...
    // handle Oppo
      ...
      uaData.device.manufacturer &&
      uaData.device.manufacturer.toLowerCase() === 'oppo' &&
      uaData.device.model
    ) {
      // 针对base库的数据做数据格式化处理
      // 解决移动联通等不同发行版导致的机型不同问题
    ...
    // handle coolpad
      ...
      uaData.device.manufacturer &&
      uaData.device.manufacturer.toLowerCase() === 'coolpad' &&
      uaData.device.model
    ) {
      // base 库适配
      // 解决移动联通等不同发行版导致的机型不同问题
    ...
    // ZTE
      ...
      uaData.device.manufacturer &&
      uaData.device.manufacturer.toLowerCase() === 'zte' &&
      uaData.device.model
    ) {
      // base 库适配
      // 解决移动联通等不同发行版导致的机型不同问题
    ...
    // handle nubia
      ...
      (match = ua.match(/(Nubia[-_\s](\w*)|(NX501|NX505J|NX506J|NX507J|NX503A|nx\d+\w*)[\s\)])/i))
    ) {
    ...
    if (uaData.device.manufacturer) {
      uaData.device.manufacturer =
        uaData.device.manufacturer.substr(0, 1).toUpperCase() +
        uaData.device.manufacturer.substr(1).toLowerCase();
    }
  ...
  Detected.prototype = {
    ...
    detect(ua) {
      ...
      if (ua.match('Android')) {
        ...
        if (
          (match = /Android(?: )?(?:AllPhone_|CyanogenMod_)?(?:\/)?v?([0-9.]+)/.exec(
            ua.replace('-update', '.')
          ))
        ) {
          this.os.version = new Version({
        ...
        if (
          (match = /Eclair; (?:[a-zA-Z][a-zA-Z](?:[-_][a-zA-Z][a-zA-Z])?) Build\/([^\/]*)\//.exec(
            ua
          ))
        ) {
          ...
          (match = /[a-zA-Z][a-zA-Z](?:[-_][a-zA-Z][a-zA-Z])?; ([^;]*[^;\s]);\s+Build/.exec(ua))
        ) {
          this.device.model = match[1];
          ...
          (match = /Android [^;]+; (?:[a-zA-Z][a-zA-Z](?:[-_][a-zA-Z][a-zA-Z])?; )?([^)]+)\)/.exec(
            ua
          ))
        ) {
      ...
      if (
        (match = /^Mozilla\/5.0 \((?:Nokia|NOKIA)(?:\s?)([^\)]+)\)UC AppleWebkit\(like Gecko\) Safari\/530$/.exec(
          ua
        ))
      ) {
        this.device.manufacturer = 'Nokia';
      ...
      if (ua.match('UCWEB')) {
        ...
        if (
          (match = /^JUC \(Linux; U; ([0-9\.]+)[^;]*; [^;]+; ([^;]*[^\s])\s*; [0-9]+\*[0-9]+\)/.exec(
            ua
          ))
        ) {
      ...
      if (this.options.detectCamouflage) {
        ...
        if (this.browser.name === 'Safari') {
          if (
            this.os.name !== 'iOS' &&
            /AppleWebKit\/([0-9]+.[0-9]+)/i.exec(ua)[1] !== /Safari\/([0-9]+.[0-9]+)/i.exec(ua)[1]
          ) {
        ...
        if (this.options.useFeatures) {
          ...
          if (window.ActiveXObject) {
            ...
            if (typeof this.engine.name !== 'undefined' && this.engine.name !== 'Trident') {
              this.camouflage =
                typeof this.browser.name === 'undefined' || this.browser.name !== 'Maxthon';
            }
          }
